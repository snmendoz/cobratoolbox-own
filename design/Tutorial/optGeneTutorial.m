...
% yeast=readCbModel('yeast_7.6_cobra');
% load('yeast7')
% [x, population, scores, optGeneSol] = optGene(yeast, 'r_1808',[],yeast.genes);
...
load('iJO1366')
model=iJO1366;
model.rules=regexprep(model.grRules,{'and','or'},{'&','|'});
genes_Involucrados_por_Rxn=regexp(regexprep(model.grRules,'\or|and|\(|\)',''),'\  ','split');
for i=1:length(model.grRules)
    for j=1:length(genes_Involucrados_por_Rxn{i})
        gen_j=genes_Involucrados_por_Rxn{i}(j);
        if isempty(gen_j{1}); continue; end; 
        pos=find(cellfun(@isempty,strfind(model.genes,gen_j{1}))==0);
        rule=['x(' num2str(pos) ')'];
        model.rules{i}=regexprep(model.rules{i},gen_j{1},rule);
...
%         optKnockSol = OptKnock(model,selectedRxnList,options,constrOpt);
        [x, population, scores, optGeneSol] = optGene(model, 'EX_lac__D_e',[],selectedGeneList);
    else
